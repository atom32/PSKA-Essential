...
resolve_paths() {
  ...
  export PSKA_RAGFLOW_OVERRIDE PSKA_RAGFLOW_SERVICE_CONF
}
...
tei_base_url_lines = []
if embedding_enabled or explicit_base_url:
    tei_base_url_lines = [f"      TEI_BASE_URL: \"{tei_base_url}\""]
...
override.write_text(
    "\n".join(
        [
            ...
            "  ragflow-cpu:",
            "    environment:",
            f"      COMPOSE_PROFILES: \"{container_profiles}\"",
            f"      TEI_MODEL: \"{tei_model}\"",
            *tei_base_url_lines,
            "    volumes:",
            f"      - \"{volume}\"",
            "  ragflow-gpu:",
