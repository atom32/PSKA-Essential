...
from pska_essential.dedup import fclones_command_path
from pska_essential.workflow import build_fake_service


def main() -> int:
    service = build_fake_service()
    with TemporaryDirectory(prefix="pska-fclones-smoke-") as temp_dir:
        root_path = Path(temp_dir) / "DuplicateRoot"
        root_path.mkdir()
        text = "# Same\n\nfclones should report this duplicate content.\n"
        ...
        root = service.source_root_register(root_path, label="fclones smoke root")
        scan = service.source_scan(root["root_id"], max_files=10, extractor="auto")
        report = service.duplicate_report({"root_ids": [root["root_id"]]}, mode="fclones_hash", limit=10)

    payload = {
        "status": report["status"],
        "provider": report["provider"],
        "fclones_path": fclones_command_path() or "",
    ...
    if report["status"] == "unavailable":
        return 77
    if report["status"] != "ok":
        raise SystemExit(f"expected ok fclones report, got {report['status']}: {report.get('message') or ''}")
    ...
    if report["data_flow"]["writes_source_files"]:
        raise SystemExit("fclones smoke must not write source files")
    return 0
