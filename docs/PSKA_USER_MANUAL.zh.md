...
- Memory 页状态区：查看 API、Memory、KB、Embedding、GBrain、Alpha、Recovery 等状态；API ready 后可点 `Mark runtime confirmed` 记录首次运行时确认，Recovery plan 加载后可点 `Mark recovery reviewed` 和 `Mark writeback locked` 记录备份/写回边界确认。
...
- Review 页面：查看候选，接受、拒绝，或把已接受候选写入长期记忆；打开候选详情后可以点 `Mark review inspected` 记录首次试用进度。
- First-run checklist：记录首次 dogfooding/alpha 试用的人工确认状态和备注，其中包括一次 Source Evidence 到记忆候选的演练；所有必选项检查后可点 `Mark exit notes` 保存退出总结。
...
- Recent Answer Proofs 在 Hermes extension 主页面展示最近回答侧工具调用 proof，并支持点开查看 trace、checks、完成的 PSKA 工具，以及从 proof 起草带来源的 Memory Review 候选。
...
`PSKA_IMPORT_REPORT.md`、注册只读资料源并建立本地检索。重复导入同一 archive
目录时，只清理带 PSKA 导入标记的旧会话、report 和 manifest，不删除无标记用户文件。
然后在 Hermes WebUI 的 PSKA Memory 页面打开 Source Evidence，按项目、时间、人物、主题搜索原文证据，
读取全文后起草候选卡。候选要人工改写、去重、检查质量、进入 Review。只有确认后的内容才写入长期记忆。
