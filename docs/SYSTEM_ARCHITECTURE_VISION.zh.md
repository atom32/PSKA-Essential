...
截至 2026-08-13，当前可验证的系统状态是 M16 source governance baseline。
...
| Duplicate Reports | 已支持 exact hash、fclones/Czkawka hash、内置 size/name/version heuristic |
...
| 方向 | 目的 |
| --- | --- |
| Duplicate heuristics refinements | 已有同名、版本号、近似大小 first-pass；后续扩展相似文本和媒体候选 |
...
- 近似查重下一步优先 text similarity，还是先做媒体候选和更强 UI 工作台？
