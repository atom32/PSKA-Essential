...
class ProductApiTests(unittest.TestCase):
    ...
    def test_static_health_ask_review_and_apply_loop(self):
        ...
        self.assertEqual(tool_policy["pska_alpha_trial_guide"]["category"], "status")
    ...
    def test_product_api_personal_source_root_scan_search_and_read(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            duplicate_review = self._post_json(
                ...
                {"scope": {"root_ids": [registered["root"]["root_id"]]}, "status": "reported"},
            )
            duplicate_mark = self._post_json(
    ...
    def test_alpha_readiness_route_reports_trial_gate(self):
        ...
        self.assertIn("persisted_first_run_checklist", checks["user_trial_ux"]["evidence"]["implemented"])
        self.assertTrue(checks["user_trial_ux"]["evidence"]["does_not_execute_backups"])
    ...
    def test_alpha_trial_guide_route_reports_guided_first_run_plan(self):
        guide = self._get_json("/api/alpha/trial-guide")["alpha_trial_guide"]
        phases = {phase["phase_id"]: phase for phase in guide["phases"]}
        actions = {action["action"]: action for action in guide["next_actions"]}

        self.assertEqual(guide["schema"], "pska.alpha_trial_guide.v1")
        self.assertEqual(guide["readiness_schema"], "pska.alpha_readiness.v1")
        self.assertEqual(guide["readiness_status"], "technical_alpha")
        self.assertEqual(guide["trial_mode"], "guided_technical_alpha")
        self.assertTrue(guide["can_start_owner_dogfooding"])
        self.assertTrue(guide["can_start_guided_trial"])
        self.assertEqual(guide["first_run_scope"]["permission_mode"], "read_only")
        self.assertIn("direct_durable_memory_write", guide["first_run_scope"]["avoid_on_first_run"])
        self.assertFalse(guide["data_flow"]["writes_source_files"])
        self.assertFalse(guide["data_flow"]["writes_memory_directly"])
        self.assertFalse(guide["data_flow"]["executes_trial_steps"])
        self.assertEqual(phases["environment"]["status"], "needs_attention")
        self.assertEqual(phases["knowledge_scope"]["status"], "ready")
        self.assertEqual(phases["writeback_pilot"]["status"], "needs_attention")
        self.assertEqual(phases["broader_alpha"]["status"], "needs_attention")
        self.assertEqual(actions["configure_live_providers"]["api"], "GET /api/runtime/diagnostics")
        self.assertEqual(actions["keep_writeback_locked"]["tool"], "pska_alpha_recovery_plan")

    def test_alpha_recovery_plan_route_reports_backup_and_restore_boundaries(self):
        plan = self._get_json("/api/alpha/recovery-plan")["alpha_recovery_plan"]
        items = {item["item_id"]: item for item in plan["backup_items"]}
        actions = {action["action"]: action for action in plan["next_actions"]}

        self.assertEqual(plan["schema"], "pska.alpha_recovery_plan.v1")
        self.assertEqual(plan["purpose"], "backup_restore_rehearsal_before_writeback_or_broader_alpha")
        self.assertEqual(plan["providers"]["kb"], "fake-kb")
        self.assertEqual(plan["providers"]["review_store"], "SQLiteReviewStore")
        ...
        self.assertEqual(items["source_registry"]["backup_method"], "sqlite_online_backup_or_copy_when_service_stopped")
        self.assertEqual(items["user_source_roots"]["required_before"], ["native_source_writeback", "obsidian_frontmatter", "obsidian_comment", "obsidian_moc"])
        self.assertFalse(plan["data_flow"]["creates_backup"])
        self.assertFalse(plan["data_flow"]["restores_data"])
        self.assertFalse(plan["data_flow"]["writes_source_files"])
        self.assertFalse(plan["data_flow"]["writes_memory_directly"])
    ...
    def test_agentic_context_brief_route_composes_recall_memory_trace_without_writes(self):
        ...
        recent = self._get_json("/api/agentic/context-briefs?limit=3")
        ...
        workflow = self.service.store.get_workflow(brief["run_id"])
    ...
    def test_bundled_diagnostic_ui_contains_reader_view(self):
        ...
        self.assertIn("refresh-review-card", styles)
        self.assertIn("refresh-review-diff", styles)
        self.assertIn("reviseMemoryReviewQueueQualityIssue", script)
        self.assertIn("memoryReviewQueueRefreshReviewCard", script)
        ...
        self.assertIn('item.item_type === "memory_refresh_review"', script)
        self.assertIn("item.previous_text", script)
        self.assertIn("item.proposed_text", script)
        ...
        self.assertIn("loadAgenticContextBriefs", script)
        self.assertIn("/api/agentic/context-brief", script)
        self.assertIn("/api/agentic/context-briefs", script)
        self.assertIn("agenticTraceItems", script)
        self.assertIn("agenticContextBriefHistory", script)
        self.assertIn("restoreAgenticContextBrief", script)
        ...
        self.assertIn('id="alpha-trial-guide"', html)
        self.assertIn('openView("settings");', script)
        self.assertIn('await loadAlphaRecoveryPlan({ silent: true });', script)
        ...
        self.assertIn(".alpha-checklist-note", styles)
        ...
        self.assertIn('item.next_review_id', script)
