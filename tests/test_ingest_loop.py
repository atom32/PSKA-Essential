...
class IngestLoopTests(unittest.TestCase):
    ...
    def test_ingest_loop_records_resumable_ask_when_uploaded_scope_is_processing(self):
        ...
        self.assertEqual(result["next_actions"][0]["tool"], "pska_kb_ingestion_status")
        self.assertEqual(result["next_actions"][1]["action"], "resume_ingest_loop")
        self.assertEqual(result["next_actions"][1]["tool"], "pska_ingest_loop_resume")
        self.assertFalse(result["next_actions"][1]["can_resume"])
    ...
    def test_ingest_loop_stops_when_ingested_scope_is_not_ready(self):
        ...
        self.assertEqual(result["next_actions"][0]["action"], "track_ingestion_status")
        self.assertEqual(result["next_actions"][0]["tool"], "pska_kb_ingestion_status")
        self.assertNotIn("delete", {action["action"] for action in result["next_actions"]})
