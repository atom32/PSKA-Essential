...
class HermesWebuiExtensionTests(unittest.TestCase):
    ...
    def test_extension_is_chip_only_and_uses_sidecar_bridge(self):
        ...
        self.assertIn("Hermes modules", script)
        self.assertIn("Apply suggested scope", script)
        self.assertIn("Sync Review Board", script)
        self.assertIn("Create Digest Task", script)
